...
describe("createChannelMetadataFilter", () => {
	it("kind:40のフィルタをチャンネルIDで生成する", () => {
		const filter = createChannelMetadataFilter();
		expect(filter.kinds).toEqual([40]);
		expect(filter.ids).toEqual([CHANNEL_ID]);
	});
});
